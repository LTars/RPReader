...
if (id) {
  if (backLink) {
    if (sessionStorage.getItem('returnLinkId')) {
      backLink.href        = BASE_URL + 'pages/reader.html';
      backLink.textContent = UI.BACK_READER;
    } else {
      backLink.href        = BASE_URL + 'pages/character.html';
...
function renderLibrary(index) {
  const items = index.map(entry => `
    <a class="char-list-item" href="${BASE_URL}pages/character.html?id=${entry.id}">
...
function renderChar(char) {
  ...
  const scenes = (char.scenes || []).map(s => `
    <a class="char-scene" href="${BASE_URL}pages/reader.html#${s.anchor}">
      ${s.title || s.id}
  ...
  const relations = (char.relations || []).map(r => `
    <a class="char-relation" href="${BASE_URL}pages/character.html?id=${r.characterId}">
